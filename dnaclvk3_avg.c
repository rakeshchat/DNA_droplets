...
int    Nr,Ng,Ny,Nyp,y,z,i,j,l,e1,ens,p,q,pp,dd,cv,rhog1,rhor1,rhoy1,igg1,irr1,irg1,ioo1,temp1,count2=0,gap,datap,r=0,s=0,u=0,v=0;
double rhor,rhog,rhoy,c,d,e,f,Ei,Ef,dE,diff,temp,igg,irr,irg,ioo=1.0,rot,clev;
char   out1[1000],out2[1000];
//init_genrand(87215);
init_genrand((unsigned long)time(NULL));
FILE   *fq,*fq2,*fr,*fread,*flistr,*flistg;


flistr  = fopen("listred_L50","w");
flistg  = fopen("listgreen_L50","w");
...
sprintf(out1,"grnCVL%d4t%dr%dg%dy%dT%drr%.2frg%.2f-%d",L,pp,rhor1,rhog1,rhoy1,temp1,irr,irg,e1); fq=fopen(out1,"w");
sprintf(out2,"redCVL%d4t%dr%dg%dy%dT%drr%.2frg%.2f-%d",L,pp,rhor1,rhog1,rhoy1,temp1,irr,irg,e1); fq2=fopen(out2,"w");

fprintf(flistg,out1); fprintf(flistg,"\n");
fprintf(flistr,out2); fprintf(flistr,"\n");
...
for(y=0;y<L;y++){for(z=0;z<L;z++){fscanf(fread,"%lld",&a[y][z]);} }
...
fprintf(fq,"\"3      c #000000\",");fprintf(fq,"\n");    //for red arms
fprintf(fq,"\"2      c #000000\",");fprintf(fq,"\n");    //for red center
fprintf(fq,"\"9      c #000000\",");fprintf(fq,"\n");    //for surf center     #d9bb14
fprintf(fq,"\"7      c #1bcb0b\",");fprintf(fq,"\n");    //for surf arms-green #1bcb0b
fprintf(fq,"\"4      c #000000\",");fprintf(fq,"\n");    //for surf arm-red    #f8100a
fprintf(fq,"\"8      c #000000\",");fprintf(fq,"\n");    //for boundary black  #6cd8fe

for(y=0;y<L;y++){fprintf(fq,"\""); for(z=0;z<L;z++){fprintf(fq,"%lld",a[y][z]); } fprintf(fq,"\","); fprintf(fq,"\n");}
//print------------------------------------------
...
fprintf(fq2,"\"%d %d 9 1\",",(int)L,(int)L);   fprintf(fq2,"\n");      //x,t,type object
fprintf(fq2,"\"0      c #000000\",");fprintf(fq2,"\n");    //empty sites #686969
fprintf(fq2,"\"6      c #000000\",");fprintf(fq2,"\n");    //for green arms
fprintf(fq2,"\"5      c #000000\",");fprintf(fq2,"\n");    //for green center
fprintf(fq2,"\"3      c #f8100a\",");fprintf(fq2,"\n");    //for red arms
fprintf(fq2,"\"2      c #f8100a\",");fprintf(fq2,"\n");    //for red center
fprintf(fq2,"\"9      c #000000\",");fprintf(fq2,"\n");    //for surf center     #d9bb14
fprintf(fq2,"\"7      c #000000\",");fprintf(fq2,"\n");    //for surf arms-green #1bcb0b
fprintf(fq2,"\"4      c #f8100a\",");fprintf(fq2,"\n");    //for surf arm-red    #f8100a
fprintf(fq2,"\"8      c #000000\",");fprintf(fq2,"\n");    //for boundary black  #6cd8fe

for(y=0;y<L;y++){fprintf(fq2,"\""); for(z=0;z<L;z++){fprintf(fq2,"%lld",a[y][z]); } fprintf(fq2,"\","); fprintf(fq2,"\n");}
//print------------------------------------------


count2 = count2 + gap;} }//Tmax ends

fclose(fq);  fclose(fq2);

             }//ens ends

fclose(flistr); fclose(flistg); fclose(fread);
