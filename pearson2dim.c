...
int    Nr,Ng,Ny,Nyp,N1,y,z,i,j,l,p,q,pp,ee,dd,cv,rhog1,rhor1,rhoy1,igg1,irr1,irg1,ioo1,temp1,count2=0,gap,datap,r=0,s=0,u=0,v=0,c1,r1,r1a,r2a,c2,r2,e1,ens;
...
sprintf(out1,"dimcorr_2L%dt%de%d",L,pp,ens); fq=fopen(out1,"w");
...
if(a[p][q]==c1){
          if(a[p][(q+2)%L]==r1 || a[p][(q+2)%L]==r1a){ds1[t][e1]++;}
          if(a[p][(q-2+L)%L]==r1 || a[p][(q-2+L)%L]==r1a){ds1[t][e1]++;}
          if(a[(p-2+L)%L][q]==r1 || a[(p-2+L)%L][q]==r1a){ds1[t][e1]++;}
          if(a[(p+2)%L][q]==r1 || a[(p+2)%L][q]==r1a){ds1[t][e1]++;}
          if(a[(p-1+L)%L][(q+1)%L]==r1 || a[(p-1+L)%L][(q+1)%L]==r1a){ds1[t][e1]++;}
          if(a[(p-1+L)%L][(q-1+L)%L]==r1 || a[(p-1+L)%L][(q-1+L)%L]==r1a){ds1[t][e1]++;}
          if(a[(p+1)%L][(q+1)%L]==r1 || a[(p+1)%L][(q+1)%L]==r1a){ds1[t][e1]++;}
          if(a[(p+1)%L][(q-1+L)%L]==r1 || a[(p+1)%L][(q-1+L)%L]==r1a){ds1[t][e1]++;}
               

          if(a[p][(q+2)%L]==r2 || a[p][(q+2)%L]==r2a){ds2[t][e1]++;}
          if(a[p][(q-2+L)%L]==r2 || a[p][(q-2+L)%L]==r2a){ds2[t][e1]++;}
          if(a[(p-2+L)%L][q]==r2 || a[(p-2+L)%L][q]==r2a){ds2[t][e1]++;}
          if(a[(p+2)%L][q]==r2 || a[(p+2)%L][q]==r2a){ds2[t][e1]++;}
          if(a[(p-1+L)%L][(q+1)%L]==r2 || a[(p-1+L)%L][(q+1)%L]==r2a){ds2[t][e1]++;}
          if(a[(p-1+L)%L][(q-1+L)%L]==r2 || a[(p-1+L)%L][(q-1+L)%L]==r2a){ds2[t][e1]++;}
          if(a[(p+1)%L][(q+1)%L]==r2 || a[(p+1)%L][(q+1)%L]==r2a){ds2[t][e1]++;}
          if(a[(p+1)%L][(q-1+L)%L]==r2 || a[(p+1)%L][(q-1+L)%L]==r2a){ds2[t][e1]++;}
               }
         }} //corr loop ends
