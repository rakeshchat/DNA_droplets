...
//Pearson2_salt, single data for L(52),Tr(6),ens(10) = 9.5 min (single data for irr,igg)
...
double rhor,rhog,rhoy,c,d,e,f,Ei,Ef,dE,diff,temp,igg,irr,irg=0.0,ioo,rot,clev,ds1,ds2,corr1,corr2;
...
igg = irr*1.035; corr1=0; corr2=0;

rhog1=rhog*1000; rhor1=rhor*1000; rhoy1=rhoy*1000; irr1=irr*10; igg1=igg*10; irg1=irg*10; ioo1=ioo*10; temp1=temp*100;

//sprintf(out1,"Salt2corrL%d_%dt%de%dr%.2fg%.2fT%d",L,qq,pp,ens,irr,igg,temp1); fq=fopen(out1,"w");
sprintf(out1,"000Salt2corrL%d_%dt%de%dT%d",L,qq,pp,ens,temp1); fq=fopen(out1,"w");

//irr, igg loop starts
for(irr = 1.2;irr < 3.2;irr += 0.3){ igg=irr*1.035; corr1=0; corr2=0;
...
for(p=0;p<L;p++){for(q=0;q<L;q++){
//for(i=0;i<L;i++){for(j=0;j<L;j++){ p = genrand_real1()*L; q = genrand_real1()*L;
