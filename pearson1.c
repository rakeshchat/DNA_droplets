...
//pearson L(50),pp(6),ens(1) = 10 min 
...
double rhor,rhog,rhoy,c,d,e,f,Ei,Ef,dE,diff,temp,igg,irr,irg=0.0,ioo,rot,clev,ds1,ds2;
...
sprintf(out1,"0corr1L%dt%de%d",L,pp,ee); fq=fopen(out1,"w");
...
if(a[p][q]==c1){
          if(a[p][(q+3)%L]==r1){ds1++;}
          if(a[p][(q-3+L)%L]==r1){ds1++;}
          if(a[(p-3+L)%L][q]==r1){ds1++;}
          if(a[(p+3)%L][q]==r1){ds1++;}
          if(a[(p-1+L)%L][(q+2)%L]==r1){ds1++;}
          if(a[(p-1+L)%L][(q-2+L)%L]==r1){ds1++;}
          if(a[(p-2+L)%L][(q+1)%L]==r1){ds1++;}
          if(a[(p-2+L)%L][(q-1+L)%L]==r1){ds1++;}
          if(a[(p+1)%L][(q+2)%L]==r1){ds1++;}
          if(a[(p+1)%L][(q-2+L)%L]==r1){ds1++;}
          if(a[(p+2)%L][(q+1)%L]==r1){ds1++;}
          if(a[(p+2)%L][(q-1+L)%L]==r1){ds1++;}
               

          if(a[p][(q+3)%L]==r2){ds2++;}
          if(a[p][(q-3+L)%L]==r2){ds2++;}
          if(a[(p-3+L)%L][q]==r2){ds2++;}
          if(a[(p+3)%L][q]==r2){ds2++;}
          if(a[(p-1+L)%L][(q+2)%L]==r2){ds2++;}
          if(a[(p-1+L)%L][(q-2+L)%L]==r2){ds2++;}
          if(a[(p-2+L)%L][(q+1)%L]==r2){ds2++;}
          if(a[(p-2+L)%L][(q-1+L)%L]==r2){ds2++;}
          if(a[(p+1)%L][(q+2)%L]==r2){ds2++;}
          if(a[(p+1)%L][(q-2+L)%L]==r2){ds2++;}
          if(a[(p+2)%L][(q+1)%L]==r2){ds2++;}
          if(a[(p+2)%L][(q-1+L)%L]==r2){ds2++;}
               }
         }} //corr loop ends
