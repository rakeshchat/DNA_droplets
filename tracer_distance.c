...
int    r=0,s=0,u=0,v=0,posx=0,posy=0,e1,ens,tim1=0,tim2=0,pt1=0,pt2=0,pt3=0,x1,y1,x2,y2;
double rhor,rhog,rhoy,c,d,e,f,Ei,Ef,dE,diff,temp,igg,irr,ioo=1.0,irg=0.0,rot=1.0,clev=1.0,dist=0;
...
sprintf(out2,"distance_ens%dL%d",ens,L); fq3=fopen(out2,"w");
...
for(y=0;y<L;y++){for(z=0;z<L;z++){if(a[y][z]==9){posx=y; posy=z; }}}
     ...
     if(pt1==0 && cls[posx][(posy+2)%L]==0 && cls[(posx+2)%L][posy]==0 && cls[posx][(posy-2+L)%L]==0 && cls[(posx-2+L)%L][posy]==0 && cls[(posx-1+L)%L][(posy-1+L)%L]==0 && cls[(posx-1+L)%L][(posy+1)%L]==0 && cls[(posx+1)%L][(posy+1)%L]==0 && cls[(posx+1)%L][(posy-1+L)%L]==0 && pt3==0){pt1=1; pt3=1; tim1=t; x1=posx; y1=posy; }
                                              
     else if(cls[posx][(posy+2)%L]==1 || cls[(posx+2)%L][posy]==1 || cls[posx][(posy-2+L)%L]==1 || cls[(posx-2+L)%L][posy]==1 || cls[(posx-1+L)%L][(posy-1+L)%L]==1 || cls[(posx-1+L)%L][(posy+1)%L]==1 || cls[(posx+1)%L][(posy+1)%L]==1 || cls[(posx+1)%L][(posy-1+L)%L]==1){if(pt1==1){tim2=t; pt1=0; pt3=0; x2=posx; y2=posy; dist=sqrt((x2-x1)*(x2-x1) + (y2-y1)*(y2-y1)); fprintf(fq3,"%lf\n",dist);}}
