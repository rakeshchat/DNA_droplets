...
double *corr2sqr; corr2sqr = (double*)calloc(Tr, sizeof(double)); for(t=0;t<Tr;t++){corr2sqr[t]=0;}
...
sprintf(out1,"zzcorrP2L%d_%dt%de%dr%dg%dy%drr%dgg%doo%dT%d",L,qq,pp,ens,rhor1,rhog1,rhoy1,irr1,igg1,ioo1,temp1); fq=fopen(out1,"w");
...
for(t=0;t<Tr;t+=gap){fprintf(fq,"%d    %lf   %lf\n",t,corr2[t]/ens,sqrt(corr2sqr[t]/ens - (corr2[t]*corr2[t]/(ens*ens))));}


fclose(fq); free(corr2); free(corr2sqr);
